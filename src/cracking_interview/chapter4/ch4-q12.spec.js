...
for (let key in funcs) {
  ...
  describe('ch4-q12: ' + key, function() {
    ...
    it('throws an error if tree is null or empty', function() {
      expect(() => func(null, 10)).toThrowError('tree must be valid and non-empty')
      expect(() => func(thisTree)).toThrowError('tree must be valid and non-empty')
    })
    ...
    it('returns correct counts with unbalanced tree', function() {
      ;[10, 8, 16, 4, 14, 22, 6, 12, 18, 5, 17, 19].forEach(v => thisTree.add(v))
      expect(func(thisTree, 10)).toBe(2)
